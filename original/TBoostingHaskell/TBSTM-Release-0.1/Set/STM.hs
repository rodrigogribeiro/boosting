...
orElse (STM t1) (STM t2) = STM $ \tstate -> do
	...
	tRes1 <- t1 tstate{undo= return (),tbUndo=return(),tbCommit=return()}
	case tRes1 of
		Retry nTState1 	-> do
				undo nTState1
				tbUndo nTState1
...
newTBSTM :: IO (Maybe a) -> (Maybe a-> IO ()) -> IO () -> STM a
newTBSTM mac undo commit = STM $ \tState -> do
	r <- mac
	case r of
		Just v -> return (Valid tState{tbUndo = (tbUndo tState) >> undo (Just v), tbCommit = (tbCommit tState) >> commit}  v)
		Nothing -> return (Invalid tState{tbUndo = (tbUndo tState) >> undo Nothing})
